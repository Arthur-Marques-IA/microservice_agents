...
| `agents/base.py` + `/analyze` | O agente de análise recebe `db=get_db()` e cada chamada cria uma sessão `analyze-<uuid>` no Postgres | Não persistir sessão em `kind="analysis"` (o trace já registra tudo) |
